...
allLogFiles () {
    ...
    do
        filesize=\$(stat -c%s "\$f")
        if [ \$filesize ]; then
            curl https://api.wafhub.com/logs -H "Authorization: Bearer $APIKEY" -F "web=nginx" -F "host=$HOSTNAME" -F "log=@\$f"
        fi
        sleep 10s
    done
    ...
    do
        filesize=\$(stat -c%s "\$f")
        if [ \$filesize ]; then
            curl https://api.wafhub.com/logs -H "Authorization: Bearer $APIKEY" -F "web=apache" -F "host=$HOSTNAME" -F "log=@\$f"
        fi
